...
describe("selected trip summary", () => {
  it("puts the trip total and base-fare saving before itinerary detail", () => {
    ...
    expect(screen.getByLabelText("LGW 08:00 to MXP 10:00")).toBeInTheDocument();
    expect(screen.getByText("Separate tickets")).toBeInTheDocument();
  });

  it("shows round-trip base totals and both component itineraries", () => {
    render(<TripSummary outbound={synthetic()} inbound={synthetic("RETURN")} outboundBaseline={option({ id: "out-direct" })} inboundBaseline={option({ id: "in-direct", direction: "RETURN" })} outboundComparisonEnabled inboundComparisonEnabled />);
  ...
  it("hides saving and extra travel in a nonstop-only compact summary", () => {
    ...
    expect(compact).toHaveTextContent("LGW-CAG");
